...
import com.codex.business.mockedAddKycDTO
import com.codex.business.mockedKycDTO
...
class KycEBVerticleTest : MockTestBase() {
    ...
    fun setUp(vertx: Vertx, ctx: VertxTestContext) {
        ...
        val options = DeploymentOptions()
            .setWorker(true)
        vertx.deployVerticle(KycEBVerticle::class.java, options, ctx.succeedingThenComplete())
    }

    @Test
    fun `it should add a record`(vertx: Vertx, ctx: VertxTestContext) {
        //GIVEN
        val addDTO = mockedAddKycDTO()
        val dto = mockedKycDTO()
        val payload = JsonObject.mapFrom(addDTO)
        val mockKycController = declareMock<KycController> {
            every { add(any()) } returns wrapSuccessInResponse(dto)
        }

        //WHEN
        val options = DeliveryOptions()
            .addHeader("action", EBAction.KycEBAction.CREATE)
        vertx.eventBus().request<JsonObject>(EBAddress.KYC, payload, options)
            .onComplete(ctx.succeeding<Message<JsonObject>> { msg ->
                val result = AppResponse.fromJson<KycDto>(msg.body())
                ctx.verify {
                    //THEN
                    verifyAll { mockKycController.add(any()) }
                    ...
                    Assertions.assertThat(result.data).isInstanceOf(KycDto::class.java)
                    ctx.completeNow()
                }
            })
    }
    ...
    fun `it should update a record`(vertx: Vertx, ctx: VertxTestContext) {
        ...
        //WHEN
        val options = DeliveryOptions()
            .addHeader("action", EBAction.KycEBAction.UPDATE)
        vertx.eventBus().request<JsonObject>(EBAddress.KYC, payload, options)
            .onComplete(ctx.succeeding<Message<JsonObject>> { msg ->
                val result = AppResponse.fromJson<KycDto>(msg.body())
                ctx.verify {
                    //THEN
                    verifyAll { mockKycController.update(any()) }
                    Assertions.assertThat(result.code).isEqualTo(CODE_SUCCESS)
                    Assertions.assertThat(result.data).isEqualTo(dto)
                    Assertions.assertThat(result).isInstanceOf(AppResponse::class.java)
                    Assertions.assertThat(result.data).isInstanceOf(KycDto::class.java)
    ...
    fun `it should get a record by id`(vertx: Vertx, ctx: VertxTestContext) {
        ...
        //WHEN
        val options = DeliveryOptions()
            .addHeader("action", EBAction.KycEBAction.GET_BY_ID)
        vertx.eventBus().request<JsonObject>(EBAddress.KYC, payload, options)
            .onComplete(ctx.succeeding<Message<JsonObject>> { msg ->
                val result: Response<KycDto> = AppResponse.fromJson(msg.body())
                ctx.verify {
                    //THEN
                    verifyAll { mockKycController.findById(any()) }
                    Assertions.assertThat(result.code).isEqualTo(CODE_SUCCESS)
                    Assertions.assertThat(result.data).isEqualTo(dto)
                    Assertions.assertThat(result).isInstanceOf(AppResponse::class.java)
                    Assertions.assertThat(result.data).isInstanceOf(KycDto::class.java)
    ...
    fun `it should delete a record by id`(vertx: Vertx, ctx: VertxTestContext) {
        ...
        //WHEN
        val options = DeliveryOptions()
            .addHeader("action", EBAction.KycEBAction.DELETE_BY_ID)
        vertx.eventBus().request<JsonObject>(EBAddress.KYC, payload, options)
            .onComplete(ctx.succeeding<Message<JsonObject>> { msg ->
                val result = AppResponse.fromJson<KycDto>(msg.body())
                ctx.verify {
                    //THEN
                    verifyAll { mockKycController.deleteById(any()) }
                    Assertions.assertThat(result.code).isEqualTo(CODE_SUCCESS)
                    Assertions.assertThat(result.data).isEqualTo(dto)
                    Assertions.assertThat(result).isInstanceOf(AppResponse::class.java)
                    Assertions.assertThat(result.data).isInstanceOf(KycDto::class.java)
    ...
    fun `it should list some record`(vertx: Vertx, ctx: VertxTestContext) {
        //GIVEN
        val pagedContent = PagedContent<KycDto>()
        pagedContent.page = 1
        ...
        pagedContent.data = generator.objects(KycDto::class.java, 1).collect(Collectors.toList())
        ...
        //WHEN
        val options = DeliveryOptions()
            .addHeader("action", EBAction.KycEBAction.LIST)
        vertx.eventBus().request<JsonObject>(EBAddress.KYC, payload, options)
            .onComplete(ctx.succeeding<Message<JsonObject>> { msg ->
                val result: Response<PagedContent<KycDto>> = AppResponse.fromJson(msg.body())
                println("Result: ${result.data}")
                println("PagedContent: $pagedContent")
                ctx.verify {
                    //THEN
                    verifyAll { mockKycController.list(any(), any()) }
                    Assertions.assertThat(result.code).isEqualTo(CODE_SUCCESS)
                    Assertions.assertThat(result).isInstanceOf(AppResponse::class.java)
    ...
    fun `it should search some record`(vertx: Vertx, ctx: VertxTestContext) {
        ...
        pagedContent.hasNextPage = true
        pagedContent.hasPreviousPage = false
        pagedContent.data = generator.objects(KycDto::class.java, 5).collect(Collectors.toList())
        ...
        //WHEN
        val options = DeliveryOptions()
            .addHeader("action", EBAction.KycEBAction.SEARCH)
        vertx.eventBus().request<JsonObject>(EBAddress.KYC, payload, options)
            .onComplete(ctx.succeeding<Message<JsonObject>> { msg ->
                val result = AppResponse.fromJson<PagedContent<KycDto>>(msg.body())
                ctx.verify {
                    //THEN
                    verifyAll { mockKycController.search(any()) }
                    Assertions.assertThat(result.code).isEqualTo(CODE_SUCCESS)
                    Assertions.assertThat(result).isInstanceOf(AppResponse::class.java)
                    Assertions.assertThat(result.data).isInstanceOf(PagedContent::class.java)
                    ctx.completeNow()
